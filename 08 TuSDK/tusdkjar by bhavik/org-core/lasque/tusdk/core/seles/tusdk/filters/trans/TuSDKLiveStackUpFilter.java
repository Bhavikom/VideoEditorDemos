...
public class TuSDKLiveStackUpFilter
  extends SelesFilter
  implements SelesParameters.FilterParameterInterface
{
  ...
  private void a()
  {
    ...
    {
      this.n = Math.abs((float)(this.m - this.o) / this.p);
      if (this.n > 1.0F) {
        this.n = 1.0F;
      } else if (this.n < 0.0F) {
        this.n = 0.0F;
    ...
    float[] arrayOfFloat3 = (float[])arrayOfFloat2.clone();
    if (Math.abs(this.n) < 0.5F)
    {
      arrayOfFloat2[0] += Math.abs(this.n);
      arrayOfFloat2[2] += Math.abs(this.n);
      arrayOfFloat2[4] += Math.abs(this.n);
      arrayOfFloat2[6] += Math.abs(this.n);
      arrayOfFloat3[0] -= Math.abs(this.n);
      arrayOfFloat3[2] -= Math.abs(this.n);
      arrayOfFloat3[4] -= Math.abs(this.n);
      arrayOfFloat3[6] -= Math.abs(this.n);
    }
    else
    {
      arrayOfFloat2[0] += 1.0F - Math.abs(this.n);
      arrayOfFloat2[2] += 1.0F - Math.abs(this.n);
      arrayOfFloat2[4] += 1.0F - Math.abs(this.n);
      arrayOfFloat2[6] += 1.0F - Math.abs(this.n);
      arrayOfFloat3[0] -= 1.0F - Math.abs(this.n);
      arrayOfFloat3[2] -= 1.0F - Math.abs(this.n);
      arrayOfFloat3[4] -= 1.0F - Math.abs(this.n);
      arrayOfFloat3[6] -= 1.0F - Math.abs(this.n);
    }
...
/* Location:              C:\Users\OM\Desktop\tusdkjar\TuSDKCore-3.1.0.jar!\org\lasque\tusdk\core\seles\tusdk\filters\trans\TuSDKLiveStackUpFilter.class
